...
class DailyOrderSummarySchedulerServiceTests {
    ...
    void setUp() throws Exception {
        Member member1 = Member.builder().email("test1@example.com").build();
        Member member2 = Member.builder().email("test2@example.com").build();

        order1 = Order.builder()
                .member(member1)  // Order의 member 객체에 member1을 설정
                .address("서울")
                .postNum("12345")
                .build();

        order2 = Order.builder()
                .member(member2)  // Order의 member 객체에 member2을 설정
                .address("서울")
                .postNum("67890")
                .build();
